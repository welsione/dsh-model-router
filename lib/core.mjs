...
// ============================================================
// 模型能力写回（面板「模型能力与请求头」卡片）纯逻辑
//
// 写回目标：宿主 llm-pi-ai 命名空间的 provider 配置——
//   模型级：providers.<pid>.models[i] 的 contextWindow/maxTokens/reasoningEfforts/input
//   供应商级：providers.<pid>.headers（每个请求附加的自定义 HTTP 头，
//             如 opencode-go 需要的 x-opencode-session，见 MissingSessionID）
// 校验最终由 llm-pi-ai 的 settings validate 钩子（assertServiceable，严格）兜底；
// 这里做插件侧的轻校验，让面板在保存前就拿到明确报错。
// ============================================================

// 宿主 llm-pi-ai 可声明的输入模态（catalog MODALITIES：text/image）。
// video 待宿主目录支持后加入（宿主 schema 会直接拒绝未知值，不可抢先写入）。
export const INPUT_MODALITIES = ['text', 'image']

// 模型级可写回的能力字段（供应商级只有 headers）
export const MODEL_CAPABILITY_KEYS = ['contextWindow', 'maxTokens', 'reasoningEfforts', 'input']

// 宿主归属的请求头：dsh-llm attributionHeaders 会以同名覆盖配置值（写了也不生效），
// 插件侧直接拒绝，避免「配置了却没效果」的静默失效。
const ATTRIBUTION_HEADERS = new Set(['user-agent'])

// 校验供应商请求头（dict：名称 → 字符串值）。返回 {ok, headers} 或 {ok:false, error}。
// 规则：名称非空且去空格后不变；名称/值能被 Fetch Headers 接受（单行值）；
// 不允许 user-agent（宿主 attribution 接管）。空 dict 合法（= 清除全部）。
export function sanitizeRequestHeaders(headers) {
  if (headers === null || typeof headers !== 'object' || Array.isArray(headers)) {
    return { ok: false, error: 'headers 必须是 { 名称: 值 } 对象' }
  }
  const out = {}
  for (const [rawName, rawValue] of Object.entries(headers)) {
    const name = String(rawName)
    if (name.trim() === '' || name.trim() !== name) {
      return { ok: false, error: `请求头名称 "${name}" 非法（不能为空，且首尾不能有空格）` }
    }
    if (ATTRIBUTION_HEADERS.has(name.toLowerCase())) {
      return { ok: false, error: `请求头 "${name}" 由宿主 attribution 管理并会覆盖配置值，不允许设置` }
    }
    const value = rawValue === undefined || rawValue === null ? '' : String(rawValue)
    try {
      new Headers([[name, value]]) // eslint-disable-line no-undef -- Node 18+/浏览器全局
    } catch {
      return { ok: false, error: `请求头 "${name}" 不是合法的 HTTP 头（名称需符合 Fetch 规范，值需为单行文本）` }
    }
    out[name] = value
  }
...
// 校验模型输入类型声明。返回 {ok, value}：
//   null / []            → {ok, value: null}（清除声明，跟随供应商默认/目录）
//   ['text', ...]        → {ok, value: 去重后的合法模态数组}
//   含未知模态 / 非数组   → {ok:false, error}
export function sanitizeInputModalities(input) {
  if (input === null) return { ok: true, value: null }
  if (!Array.isArray(input)) return { ok: false, error: 'input 必须是数组或 null' }
  const seen = new Set()
  const out = []
  for (const item of input) {
    if (typeof item !== 'string' || !INPUT_MODALITIES.includes(item)) {
      return { ok: false, error: `输入类型仅支持 ${INPUT_MODALITIES.join('/')}（宿主暂不支持其它模态）` }
    }
    if (!seen.has(item)) { seen.add(item); out.push(item) }
  }
  if (out.length === 0) return { ok: true, value: null }
  return { ok: true, value: out }
}

// 对某 provider 的 models 列表应用一次模型级能力补丁（纯函数，不改入参）。
// patch 值为 null → 删除该字段（声明回退目录）；reasoningEfforts 为空对象 → 同删除
// （修复旧版把 {} 写进配置被宿主 assertServiceable 拒绝的问题）。
// 返回 {ok, models}（新数组）或 {ok:false, error}。
export function applyModelCapabilityPatch(models, modelId, patch) {
  if (!Array.isArray(models)) return { ok: false, error: 'models 必须是数组' }
  if (typeof modelId !== 'string' || modelId === '') return { ok: false, error: 'model 必填' }
  if (patch === null || typeof patch !== 'object' || Array.isArray(patch)) {
    return { ok: false, error: 'patch 必须是对象' }
  }
  const idx = models.findIndex((m) => m && m.id === modelId)
  if (idx < 0) return { ok: false, error: `model ${modelId} 不在 provider 的 models 列表中` }
  const target = { ...models[idx] }
  for (const [key, value] of Object.entries(patch)) {
    if (!MODEL_CAPABILITY_KEYS.includes(key)) {
      return { ok: false, error: `不支持的能力字段 "${key}"（允许：${MODEL_CAPABILITY_KEYS.join('/')}）` }
    }
    if (value === undefined) continue
    if (key === 'contextWindow' || key === 'maxTokens') {
      const n = Number(value)
      if (!Number.isInteger(n) || n <= 0) return { ok: false, error: `${key} 必须是正整数` }
      target[key] = n
    } else if (key === 'reasoningEfforts') {
      if (value === null || (typeof value === 'object' && !Array.isArray(value) && Object.keys(value).length === 0)) {
        delete target.reasoningEfforts
        continue
      }
      if (typeof value !== 'object' || Array.isArray(value)) {
        return { ok: false, error: 'reasoningEfforts 必须是对象或 null' }
      }
      for (const [level, wire] of Object.entries(value)) {
        if (wire !== null && typeof wire !== 'string') {
          return { ok: false, error: `reasoningEfforts.${level} 必须是字符串或 null` }
        }
      }
      target.reasoningEfforts = { ...value }
    } else if (key === 'input') {
      const r = sanitizeInputModalities(value)
      if (!r.ok) return r
      if (r.value === null) delete target.input
      else target.input = r.value
    }
  }
