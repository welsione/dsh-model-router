...
//
// 设计边界：
// 1) 只用于剥离“供应商把思考以 <thinking> 标签文本形式放进正文”的情况；
//    思考块（reasoning-delta）由宿主原生渲染，不在此处理，避免误伤思考内容
//    里的合法 '<'（数学/代码/泛型）。
// 2) carry 只在同类文本 chunk 之间续接；跨 chunk 类型（如思考块→正文）必须
//    重置，防止把半截标签残留拼进下一块正文（正文开头出现 '<' 的偶发污染源）。
// ------------------------------------------------------------------
export function makeThinkingTagStripper() {
  let carry = ''
  let lastKind = null // 'text' | 'reasoning' | null
  ...
  return function clean(text, kind = 'text') {
    // 跨块类型切换：清空 carry（半截标签不得进入下一块）
    if (lastKind !== null && kind !== lastKind) carry = ''
    lastKind = kind
    let input = carry + text
    let output = input.replace(TAG_RE, '')
    output = output.replace(BARE_RESPONSE_RE, '')
    // 仅当结尾确实是「标签残片」才续接：`<` 后必须紧跟字母且属于标签名起始，
    // 或已是明确的标签前缀（</thin 等）。普通数学/代码的 "<"（如 "a < b"、
    // "<T>" 的孤立开头）不匹配，直接留给文本本身。
    const tail = output.match(/<\/?(?:think|resp|reason|answer|antml)[a-zA-Z:]*$/)
      || output.match(/<[a-zA-Z][a-zA-Z:]*$/)
    if (tail) {
      carry = tail[0]
