...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function OverlayStatus(props) {
      ...
      // 若作为最新状态会把显示清空导致徽章闪烁消失。按 seq 升序遍历，
      // 后写覆盖先写，天然得到最新状态：请求进行中最新是 started（高亮），
      // 完成后最新是 served（熄灭）——与轮询路径（history）判定完全一致。
      ...
      React.useEffect(function () {
        ...
        var tick = function () {
          ...
          api('/api/model-router/state').then(function (res) {
                ...
                for (var k in routes) {
                  ...
                  for (var ti = 0; ti < TIER_ORDER.length; ti++) {
                    ...
                    for (var ci = 0; ci < chain.length; ci++) {
                      ...
                      var modelHit = cur ? cand.model === cur : slot === (manual || 'tier2')
                      if (modelHit) {
                        hit = { provider: cand.provider, model: cand.model, effort: cand.reasoningEffort || null, tier: slot, pkg: k, manual: !!manual }
                        break
                      }
                    }
                    if (hit) break
            ...
            // 取最新一条状态记录。与 deriveFromSession 判定一致：最新是 started
            // （运行中）→ 高亮，最新是 served → 熄灭，避免两路径高亮打架。
            var entry = mine.length > 0 ? mine[mine.length - 1] : null
            if (entry) {
              setLatest(entry)
            }
            setErr(null)
      ...
      // 思考级别：纯配置驱动（设置页路由配置），不依赖路由事件
      var eff = (cfgInfo && cfgInfo.effort) || null
      if (!latest) {
        // 无请求历史：展示配置信息（供应商/模型 + 思考级别，均来自设置页路由配置）
        if (!cfgInfo) return null
        return React.createElement('div', { className: 'dsh-mr-overlay', title: cfgInfo.provider + '/' + cfgInfo.model + (eff ? ' · 思考 ' + eff : '') },
          React.createElement('span', { className: 'dsh-mr-overlay-who' }, cfgInfo.provider + '/' + cfgInfo.model),
