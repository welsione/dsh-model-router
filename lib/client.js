...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Panel() {
      ...
      var card = function (key, titleNode, extraHead, body, bodyAlways) {
        var open = isOpen(key)
        return React.createElement('div', { className: 'dsh-mr-card' },
          React.createElement('button', {
            type: 'button', className: 'dsh-mr-card-head dsh-mr-card-head-btn',
            'aria-expanded': bodyAlways ? undefined : open,
            onClick: bodyAlways ? undefined : function () { toggleCard(key) },
          },
            React.createElement('b', null, titleNode),
            extraHead || null,
            !bodyAlways && chevron(open)),
          bodyAlways
            ? body
      ...
      return React.createElement('div', { className: 'dsh-mr' },
        ...
        // ---- 冷却中候选（默认折叠 · 头部计数；清空按钮在展开区内）----
        card('cooldowns', t('cd.title'),
          cooldownList.length > 0
            ? React.createElement('span', { className: 'dsh-mr-count-badge is-active' }, cooldownList.length)
        ...
        card('global', t('global.title'),
          [
            // 总开关常驻头部：折叠时也能一键启停。head 是 <button>（嵌套 button 非法），
            // 故用 span + role="switch" + tabIndex（Enter/Space 触发，a11y 等效）。
            React.createElement('span', { key: 'switch', className: 'dsh-mr-switch' + ((cfg && cfg.enabled) ? ' is-on' : ''),
              role: 'switch', tabIndex: 0, 'aria-checked': !!(cfg && cfg.enabled), 'aria-label': t('global.master'), title: t('global.master'),
              onClick: function (e) { e.stopPropagation(); setCfg(function (p) { return Object.assign({}, p, { enabled: !p.enabled }) }) },
              onKeyDown: function (e) {
                if (e.key === ' ' || e.key === 'Enter') { e.preventDefault(); e.stopPropagation(); setCfg(function (p) { return Object.assign({}, p, { enabled: !p.enabled }) }) }
              } }),
            React.createElement('span', { className: 'dsh-mr-hint', key: 'state', title: t('global.disabledTip') }, (cfg && cfg.enabled) ? t('global.on') : t('global.off')),
            globalSummary,
          ],
          [
        ...
        // ---- 模型能力 + 供应商请求头（写回宿主 llm-pi-ai · 默认折叠）----
        card('caps', t('caps.title'),
          [
            React.createElement('span', { className: 'dsh-mr-count-badge', key: 'n' },
        ...
        card('history', t('hist.title'),
          historyList.length > 0
            ? React.createElement('span', { className: 'dsh-mr-count-badge' }, historyList.length)
    ...
    function apply(ctx) {
      ...
      styleEl.textContent = [
        ...
        '.dsh-mr-card-body { display: flex; flex-direction: column; gap: 10px; }',
        '.dsh-mr-card-head { display: flex; align-items: center; gap: 8px; }',
        // 可折叠卡片头：整行可点（button 复位默认样式），chevron 推到右侧
        'button.dsh-mr-card-head-btn { width: 100%; margin: -14px; padding: 14px; border: none; background: none; color: inherit; text-align: left; cursor: pointer; border-radius: 12px; font: inherit; }',
        ...
        // 头部计数徽章（冷却候选/事件历史条数）
        '.dsh-mr-count-badge { min-width: 18px; padding: 0 6px; border-radius: 999px; text-align: center; font-size: 11px; line-height: 18px; font-weight: 600; color: var(--dsw-alias-label-secondary); background: var(--dsw-alias-bg-layer-2); flex: none; }',
        // 有内容的计数徽章用暖色提醒（如冷却中/历史条数）
        '.dsh-mr-count-badge.is-active { color: #A05A00; background: #FFF1E0; }',
        // 面板头：标题/副标题居左，保存状态居右常驻
        '.dsh-mr-headrow { display: flex; align-items: flex-end; gap: 12px; }',
