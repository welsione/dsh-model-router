...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Panel() {
      ...
      var chainEditor = function (id, slot, label, hint) {
        ...
        var effortOptions = function (c) {
          return (efforts[c.provider + '/' + c.model] || []).slice()
        }
            ...
            : chain.map(function (c, i) {
                ...
                return React.createElement('div', { className: 'dsh-mr-cand', key: i },
                  ...
                  React.createElement('select', {
                    className: 'dsh-mr-sel', value: c.provider,
                    onChange: function (e) {
                  ...
                  React.createElement('select', {
                    className: 'dsh-mr-sel dsh-mr-sel-model', value: c.model,
                    onChange: function (e) { update(i, { model: e.target.value }) },
                  }, modelOptions(c.provider, c.model).map(function (m) {
                    return React.createElement('option', { key: m, value: m }, m)
                  })),
                  React.createElement('select', {
                    className: 'dsh-mr-sel dsh-mr-sel-effort', title: '思考级别',
                    ...
                    React.createElement('option', { key: 'default', value: '' }, opts.length === 0 ? '不支持思考级别' : '默认'),
                  ].concat(opts.map(function (ef) {
                    return React.createElement('option', { key: ef.id, value: ef.id }, ef.name || ef.id)
                  }))),
                  React.createElement('span', { className: 'dsh-mr-btns' },
                    React.createElement('button', { type: 'button', className: 'dsh-mr-mini', title: '上移', disabled: i === 0, onClick: function () { move(i, -1) } }, '↑'),
                    React.createElement('button', { type: 'button', className: 'dsh-mr-mini', title: '下移', disabled: i === chain.length - 1, onClick: function () { move(i, 1) } }, '↓'),
                    React.createElement('button', { type: 'button', className: 'dsh-mr-mini dsh-mr-danger', title: '删除', onClick: function () {
      ...
      return React.createElement('div', { className: 'dsh-mr' },
        ...
        notice !== null
          ? React.createElement('div', { className: 'dsh-mr-ok' }, notice)
          : null,

        // ---- 全局设置 ----
        React.createElement('div', { className: 'dsh-mr-card' },
          React.createElement('div', { className: 'dsh-mr-card-head' },
            React.createElement('span', { className: 'dsh-mr-switch' + ((cfg && cfg.enabled) ? ' is-on' : ''),
              role: 'switch', 'aria-checked': !!(cfg && cfg.enabled), title: '总开关',
        ...
        React.createElement('div', { className: 'dsh-mr-card' },
                ...
                return React.createElement('div', { className: 'dsh-mr-route', key: id },
                  React.createElement('div', { className: 'dsh-mr-route-head' },
                    ...
                    st ? React.createElement('span', { className: 'dsh-mr-hint' },
                      st.requests + ' 请求' + (st.failovers > 0 ? ' / ' + st.failovers + ' 切换' : '')) : null,
          ...
          React.createElement('div', { className: 'dsh-mr-newroute' },
            React.createElement('input', { className: 'dsh-mr-text', placeholder: '新统一模型 ID，如 deepseek-v4-flash',
    ...
    function apply(ctx) {
      ...
      styleEl.textContent = [
        '.dsh-mr { display: flex; flex-direction: column; gap: 14px; max-width: 720px; width: 100%; color: var(--dsw-alias-label-primary); }',
        '.dsh-mr h2 { margin: 0; font-size: 16px; font-weight: 600; }',
        '.dsh-mr-sub { margin: 2px 0 0; font-size: 12px; color: var(--dsw-alias-label-secondary); line-height: 1.6; }',
        ...
        '.dsh-mr-hint { font-size: 11px; color: var(--dsw-alias-label-secondary); }',
        ...
        '.dsh-mr-switch { position: relative; flex: none; width: 36px; height: 20px; border-radius: 999px; border: none; background: var(--dsw-alias-border-l2); cursor: pointer; transition: background .15s ease; padding: 0; display: inline-block; }',
        ...
        '.dsh-mr-cd { display: flex; align-items: center; justify-content: space-between; gap: 10px; font-size: 12px; padding: 6px 10px; border: 1px solid var(--dsw-alias-border-l1); border-radius: 8px; background: var(--dsw-alias-bg-layer-2); }',
        ...
        '.dsh-mr-cand { display: grid; grid-template-columns: 22px minmax(90px, 1.1fr) minmax(110px, 1.5fr) 120px auto; align-items: center; gap: 6px; padding: 2px 4px; min-width: 0; }',
        ...
        '.dsh-mrp-menu { z-index: 20; min-width: 280px; max-width: min(340px, 100vw - 32px); max-height: min(360px, 100vh - 96px); overflow-y: auto; border: 1px solid var(--dsw-alias-border-inverted); background: var(--dsw-specific-menu); box-shadow: var(--dsw-shadow-lv3); color: var(--dsw-alias-label-primary); border-radius: 12px; flex-direction: column; padding: 4px; display: flex; position: absolute; bottom: calc(100% + 8px); right: 0; }',
