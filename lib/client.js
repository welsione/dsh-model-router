...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Panel() {
      ...
      // 档位徽章：配色固定（pro/normal/lite 色系），显示名取该套餐的 tierNames（自定义优先）。
      var tierBadge = function (routeId, slot) {
        var base = { tier3: { text: 'pro', cls: 'dsh-mr-badge-pro' }, tier2: { text: 'normal', cls: 'dsh-mr-badge-normal' }, tier1: { text: 'lite', cls: 'dsh-mr-badge-lite' } }[slot] || { text: slot, cls: '' }
        var rt = (routes && routes[routeId]) || {}
        var custom = rt.tierNames && rt.tierNames[slot]
        return { text: custom || base.text, cls: base.cls }
      }

      // 候选行编辑器
      var chainEditor = function (id, slot, label, hint) {
        var chain = (routes[id] && routes[id][slot]) || []
        var badge = tierBadge(id, slot)
      ...
      return React.createElement('div', { className: 'dsh-mr' },
        React.createElement('h2', null, '模型路由'),
        React.createElement('p', { className: 'dsh-mr-sub', title: '统一 ModelID 套餐：一个逻辑模型名汇聚多家供应商额度，首 token 前失败自动切换候选（带冷却）。三档 pro / normal / lite，每个候选可配思考级别。配置保存后即时生效。' },
          '统一模型路由 · 故障转移 · 思考级别'),
        ...
        React.createElement('div', { className: 'dsh-mr-card' },
          ...
          React.createElement('div', { className: 'dsh-mr-grid' },
            React.createElement('label', { title: '失败候选的冷却时长，毫秒；5 分钟 = 300000' }, '失败冷却',
          ...
          Object.keys(routes).length === 0
            ...
            : Object.keys(routes).sort().map(function (id) {
                ...
                return React.createElement('div', { className: 'dsh-mr-route', key: id },
                  ...
                  // 套餐级档位名称：每个套餐单独设置三档显示名
                  React.createElement('div', { className: 'dsh-mr-subsec', title: '该套餐三档的显示名（对话窗口与设置面板展示），清空恢复默认' },
                    React.createElement('span', { className: 'dsh-mr-subsec-label' }, '档位名称'),
                    TIER_ORDER2.map(function (slot) {
                      var dflt = { tier3: 'pro', tier2: 'normal', tier1: 'lite' }[slot]
                      var rt = (routes && routes[id]) || {}
                      var cur = (rt.tierNames && rt.tierNames[slot]) || dflt
                      return React.createElement('label', { key: slot, title: '档位 ' + slot + '（默认 ' + dflt + '）' },
                        React.createElement('span', { className: 'dsh-mr-tiername-label' }, cur),
                        React.createElement('input', { className: 'dsh-mr-text dsh-mr-tiername-input', type: 'text',
                          name: 'routeTierName-' + id + '-' + slot, autocomplete: 'off', spellCheck: false, translate: 'no',
                          placeholder: dflt,
                          value: (rt.tierNames && rt.tierNames[slot]) || '',
                          onChange: function (e) {
                            var v = e.target.value.trim()
                            setCfg(function (prev) {
                              var next = JSON.parse(JSON.stringify(prev))
                              var r = next.routes[id] || {}
    ...
    function OverlayStatus(props) {
      ...
      if (!full) return null
      return React.createElement('div', { className: 'dsh-mr-overlay' + (active ? ' dsh-mr-overlay-active' : ''), title: full + (eff ? ' · 思考 ' + eff : '') },
    ...
    function apply(ctx) {
      ...
      styleEl.textContent = [
        ...
        '.dsh-mr-tiername-label { font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: 12px; font-weight: 600; }',
