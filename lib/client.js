...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function PackageSelect({ available, directory, load, select, sessionId }) {
      ...
      var tierName = TIER_META[effectiveTier] || effectiveTier
    ...
    function OverlayStatus(props) {
      ...
      // 供应商/模型 + 思考级别：优先用路由事件（运行时实际值），缺少时用设置配置（cfgInfo）
      var eff = (latest && latest.reasoningEffort) || (cfgInfo && cfgInfo.effort) || null
      if (!latest) {
