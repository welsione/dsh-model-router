  ...
  factory: (require) => {
    ...
    function Panel() {
      ...
      var load = React.useCallback(function () {
        ...
        api('/api/model-router/state').then(function (res) {
          ...
          setCfg({ enabled: res.config.enabled, cooldownMs: res.config.cooldownMs,
                   ...
                   routeEventPersistence: res.config.routeEventPersistence === 'live' ? 'live' : 'session',
      ...
      var canonicalCfg = function (c) {
        ...
        return JSON.stringify({
          ...
          routeEventPersistence: c.routeEventPersistence === 'live' ? 'live' : 'session',
          routes: routes,
        })
      }
      ...
      var clearCooldowns = function () {
        ...
        api('/api/model-router/cooldowns/clear', {}).then(function () { return load() })
          .catch(function (e) { setNoticeErr(true); setNotice('清除失败：' + String((e && e.message) || e)) })
      }
      ...
      var renderCapsBody = function () {
        ...
        if (provs.length === 0) {
          return React.createElement('div', { className: 'dsh-mr-empty',
            title: '未配置自定义供应商（hand-declared）。内置目录供应商的模型能力由宿主模型目录管理。' },
            '暂无自定义供应商')
        }
      ...
      var chainEditor = function (id, slot, label, hint) {
        ...
        return React.createElement('div', { className: 'dsh-mr-chain', key: id + '.' + slot },
          ...
          chain.length === 0
            ? React.createElement('div', { className: 'dsh-mr-empty', title: '空：命中该链时回退到上一档' }, '空')
      ...
      var typeLabel = { started: '尝试', failover: '切换', served: '服务', 'all-failed': '全失败', passthrough: '放行', 'manual-tier': '手动档', 'cooldowns-cleared': '清冷却', 'skipped-context': '跳过(窗口)' }

      return React.createElement('div', { className: 'dsh-mr' },
        React.createElement('h2', null, '模型路由'),
        React.createElement('p', { className: 'dsh-mr-sub', title: '统一 ModelID 套餐：一个逻辑模型名汇聚多家供应商额度，首 token 前失败自动切换候选（带冷却）。三档 pro / normal / lite，每个候选可配思考级别。所有修改自动保存、即时生效。' },
          '统一模型路由 · 故障转移 · 思考级别'),
        ...
        React.createElement('div', { className: 'dsh-mr-card' },
          React.createElement('div', { className: 'dsh-mr-card-head' },
            ...
            React.createElement('span', { className: 'dsh-mr-hint', title: '已停用：全部请求走原路径' }, (cfg && cfg.enabled) ? '已启用' : '已停用')),
          React.createElement('div', { className: 'dsh-mr-grid' },
            React.createElement('label', { title: '冷却基础时长，毫秒；5 分钟 = 300000。限流类按 0.2 倍、服务端类按 0.5 倍缩短，AUTH 等硬失败用满额' }, '失败冷却',
            ...
            React.createElement('label', { title: '连续失败冷却退避倍数（1-16），如 2 = 30 秒基础 × 2^连续失败次数；设为 1 关闭退避' }, '冷却退避',
            ...
            React.createElement('label', { title: '冷却退避封顶（毫秒），默认 30 分钟 = 1800000' }, '冷却封顶',
            ...
            React.createElement('label', { title: '每个 step 最多切换候选次数（1-10）' }, '最多切换',
          ...
          React.createElement('label', { className: 'dsh-mr-check', title: '限流/配额/服务端/超时等瞬时错误，先重试当前候选再切换（避免一限流就立刻冷却）' },
            ...
            ' 瞬时错误重试'),
          React.createElement('div', { className: 'dsh-mr-grid' },
            React.createElement('label', { title: '瞬时错误最多重试次数（0-5）；重试耗尽才冷却并切换候选' }, '重试次数',
            ...
            React.createElement('label', { title: '重试间隔（毫秒），线性退避：第 n 次等待 n×此值' }, '重试间隔',
          ...
          React.createElement('label', { className: 'dsh-mr-check', title: '稳定成功的候选优先，频繁失败的候选后移' },
            ...
            ' 健康度择优'),
          React.createElement('label', { className: 'dsh-mr-check', title: '按请求体量（与宿主 token-meter 同标尺 chars/4 估算）跳过上下文窗口装不下的候选——大会话不再每次先打小窗口候选失败一次才切换（如 465K 会话跳过 256K 的 k3-256k 直连 1M 的 glm-5.3）。跳过不算失败、不进冷却。' },
          ...
          (cfg && cfg.contextAware !== false) ? React.createElement('div', { className: 'dsh-mr-grid' },
            React.createElement('label', { title: '窗口可用比例（0.5-1）：请求估算 > 候选窗口 × 此值 即跳过。留余量给输出与系统提示' }, '窗口余量',
            ...
            React.createElement('label', { title: '输出预留 token：叠加在输入估算之上参与窗口比较（候选未声明 maxTokens 时的固定预留）' }, '输出预留',
          ...
          React.createElement('label', { className: 'dsh-mr-check', title: '开启（默认）：把路由决策写入会话日志，客户端经会话事件流实时消费、可回放单场会话的路由轨迹；卸载本插件后旧事件需带 ignorable 标记才能被读取。关闭：不落会话日志（仅保留面板全局历史），彻底规避卸载兼容问题与日志噪音，但实时徽章退化为 2s 轮询、无法回放轨迹。' },
            React.createElement('input', { type: 'checkbox', checked: cfg ? cfg.routeEventPersistence !== 'live' : true,
              onChange: function (e) { setCfg(function (p) { return Object.assign({}, p, { routeEventPersistence: e.target.checked ? 'session' : 'live' }) }) } }),
            ' 路由事件写入会话日志'),
          React.createElement('div', { className: 'dsh-mr-actions' },
            React.createElement('span', { className: 'dsh-mr-savestatus', role: 'status', 'aria-live': 'polite' },
              saving ? '保存中…' : (lastSavedAt ? '修改自动保存 · 已保存 ' + fmtTime(lastSavedAt) : '修改自动保存')))),
        ...
        React.createElement('div', { className: 'dsh-mr-card' },
          React.createElement('div', { className: 'dsh-mr-card-head' },
            ...
            cooldownList.length > 0
              ...
              : React.createElement('span', { className: 'dsh-mr-hint', title: '候选失败后进入冷却期（按失败类型分级：限流短、服务端中、认证长，连续失败指数退避）。冷却结束或下次请求遍历时自动清理。' }, '当前无')),
          cooldownList.map(function (c) {
            var reason = c.code ? (c.code + (c.status ? ' HTTP ' + c.status : '')) : '未知错误'
            var detailTitle = '失败原因：' + reason +
              (c.streak > 0 ? '（连续失败 ' + (c.streak + 1) + ' 次，冷却按 ×' + '退避增长）' : '') +
              (c.durationMs ? '；本次冷却 ' + fmtRemaining(c.durationMs) : '')
        ...
        React.createElement('div', { className: 'dsh-mr-card' },
          React.createElement('div', { className: 'dsh-mr-card-head' },
            ...
            React.createElement('span', { className: 'dsh-mr-hint', title: capsWritable ? '写回宿主 llm-pi-ai 配置，热重载生效' : '宿主配置不可写' },
              capsWritable ? '' : '只读')),
        ...
        React.createElement('div', { className: 'dsh-mr-card' },
          ...
          Object.keys(routes).length === 0
            ? React.createElement('div', { className: 'dsh-mr-empty', title: '在对话窗口的套餐选择器中添加，或在此手动添加同名统一模型 ID' },
                '暂无套餐')
                ...
                return React.createElement('div', { className: 'dsh-mr-route', key: id },
                  React.createElement('div', { className: 'dsh-mr-route-head' },
                    ...
                    st ? React.createElement('span', { className: 'dsh-mr-hint',
                      title: '请求 ' + st.requests + ' 次' + (st.failovers > 0 ? '，切换 ' + st.failovers + ' 次（切换率 ' + (failRate || '0%') + '）' : '') },
                      st.requests + ' 请求' + (st.failovers > 0 ? ' · 切 ' + st.failovers : '')) : null,
                    React.createElement('button', { type: 'button', className: 'dsh-mr-mini dsh-mr-danger', title: '删除此路由',
                      onClick: function () { removeRoute(id) } }, '删除')),
            ...
            ? React.createElement('table', { className: 'dsh-mr-table' },
                ...
                React.createElement('tbody', null, historyList.slice(0, 10).map(function (h, i) {
                  var detail = h.type === 'failover'
                    ...
                    : h.type === 'skipped-context' ? '窗口装不下，跳过 ' + (h.try || '')
    ...
    function TierNamePill({ rid, slot, dflt, value, onCommit }) {
      ...
      return React.createElement('input', {
        ...
        placeholder: dflt, 'aria-label': '档位 ' + slot + ' 显示名',
        value: draft,
                    ...
                    displayKey
                      ? React.createElement('div', { className: 'dsh-mrp-tiers' },
                          ...
                          TIER_ORDER.map(function (slot) {
                            ...
                            var label = tierName(slot) + (cands[0] ? '' : '（未配置）')
                            return React.createElement('button', {
    ...
    function OverlayStatus(props) {
      ...
      // 最新的 model-router/route「状态」事件（started/served/all-failed）。
      // 只认这三类：manual-tier 只是用户切档的操作记录，不携带 try/by，
      // 若作为最新状态会把显示清空导致徽章闪烁消失。
      // 关键：不能依赖 Map 插入序当 seq 序——长会话（如本仓库 20 万+ seq）
      // 客户端分页回填历史事件时，旧事件可能插在新事件之后，按下标/遍历
      // 序取「最后一条」会拿到昨日残留的 tier2 事件，与轮询路径的真最新
      // 事件来回覆盖（徽章在两个模型间跳动的根因之一）。这里用事件自带
      // 的 ts（服务端 Date.now()）+ seq 键显式取最大，与插入序解耦。
      function deriveFromSession(session) {
        try {
          ...
          var lastTs = -1
          var lastSeq = -1
          inputs.forEach(function (entry, seq) {
            var ev = entry && entry.event
            if (!ev || ev.type !== 'model-router/route') return
            var d = ev.data || {}
            if (d.type !== 'started' && d.type !== 'served' && d.type !== 'all-failed') return
            var t = typeof d.ts === 'number' ? d.ts : 0
            var s = typeof seq === 'number' ? seq : (Number(seq) || 0)
            if (!lastEv || t > lastTs || (t === lastTs && s > lastSeq)) {
              lastEv = d; lastTs = t; lastSeq = s
            }
          })
          if (!lastEv) return null
          return { latest: lastEv, active: lastEv.type === 'started' }
        } catch (e) {
          return null
        }
      }
      ...
      React.useEffect(function () {
        ...
        // 始终启动轮询作为 fallback：streaming 期间 session 事件流可能不实时
        // 更新 OverlayStatus（视图快照延迟），轮询确保有数据。
        tick()
        return function () {
          alive = false
      ...
      if (!latest) {
        ...
        return React.createElement('div', { className: 'dsh-mr-overlay', title: f0 + (fallback0.effort ? ' · 思考 ' + fallback0.effort : '') },
          React.createElement('span', { className: 'dsh-mr-overlay-who' }, f0),
          fallback0.effort ? React.createElement('span', { className: 'dsh-mr-overlay-effort', title: '思考级别：' + fallback0.effort }, fallback0.effort) : null)
      }
      ...
      if (!full) return null
      return React.createElement('div', { className: 'dsh-mr-overlay dsh-mr-overlay-active', title: full + (eff ? ' · 思考 ' + eff : '') },
        React.createElement('span', { className: 'dsh-mr-overlay-who' }, full),
        eff ? React.createElement('span', { className: 'dsh-mr-overlay-effort', title: '思考级别：' + eff }, eff) : null)
    }
        ...
        scope.slots.inject('conversation.input.left', function () {
          return scope.slots.register({
            ...
            inject: function (sessionId) {
              ...
              return { sessionId: sessionId, sessions: sessions, directory: directory }
            },
