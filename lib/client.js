...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var DICT_ZH = {
      ...
      'global.master': '总开关',
      'global.title': '全局设置',
      'strip.requests': '{n} 次请求',
      'strip.switches': '{n} 次切换',
      'strip.cooldowns': '{n} 冷却中',
    ...
    var DICT_EN = {
      ...
      'strip.requests': '{n} requests',
      'strip.switches': '{n} switches',
      'strip.cooldowns': '{n} cooling',
      ...
      'caps.title': 'Provider management',
    ...
    function Panel() {
      ...
      var chainEditor = function (id, slot, label, hint) {
        ...
        var tierCls = { tier3: 'is-tier3', tier2: 'is-tier2', tier1: 'is-tier1' }[slot] || ''
        ...
        return React.createElement('div', { className: 'dsh-mr-chain ' + tierCls, key: id + '.' + slot },
          ...
          chain.length === 0
            ? React.createElement('div', { className: 'dsh-mr-chain-empty', title: t('chain.empty.tip') },
                React.createElement('span', { className: 'dsh-mr-hint' }, t('chain.empty')),
                React.createElement('button', { type: 'button', className: 'dsh-mr-mini', onClick: function () { addCandidate() } }, t('chain.add')))
      ...
      var card = function (key, titleNode, extraHead, body) {
        var open = isOpen(key)
        return React.createElement('div', { className: 'dsh-mr-card' + (open ? ' is-open' : '') },
          React.createElement('button', {
      ...
      // 运行统计（状态条 chips 用）
      var totalRequests = Object.values(statsMap).reduce(function (a, s) { return a + s.requests }, 0)
      var totalSwitches = Object.values(statsMap).reduce(function (a, s) { return a + s.failovers }, 0)
      ...
      return React.createElement('div', { className: 'dsh-mr' },
        ...
        // ---- 状态条（常驻 · 仪表盘层）----
        React.createElement('div', { className: 'dsh-mr-strip' + (enabled ? '' : ' is-off') },
          React.createElement('span', { className: 'dsh-mr-strip-state' + (enabled ? ' is-on' : '') }, enabled ? t('global.on') : t('global.off')),
          React.createElement('span', { className: 'dsh-mr-strip-sep' }, '·'),
          React.createElement('span', { className: 'dsh-mr-strip-stat' }, t('strip.requests', { n: totalRequests })),
          React.createElement('span', { className: 'dsh-mr-strip-sep' }, '·'),
          React.createElement('span', { className: 'dsh-mr-strip-stat' }, t('strip.switches', { n: totalSwitches })),
          React.createElement('span', { className: 'dsh-mr-strip-sep' }, '·'),
          React.createElement('span', { className: 'dsh-mr-strip-stat' + (cooldownList.length > 0 ? ' dsh-mr-strip-stat-warn' : '') },
            t('strip.cooldowns', { n: cooldownList.length })),
          !enabled ? React.createElement('span', { className: 'dsh-mr-hint' }, t('global.disabledTip')) : null),
        ...
        card('caps', t('caps.title'),
          ...
          [
            addProvOpen
              ...
              : null,
            React.createElement('div', { className: 'dsh-mr-provtoolbar', key: 'bar' },
              React.createElement('span', { className: 'dsh-mr-hint' },
    ...
    function apply(ctx) {
      ...
      styleEl.textContent = [
        '.dsh-mr { display: flex; flex-direction: column; gap: 14px; max-width: 860px; width: 100%; margin: 0 auto; color: var(--dsw-alias-label-primary); touch-action: manipulation; -webkit-tap-highlight-color: transparent; }',
        ...
        '.dsh-mr-card { display: flex; flex-direction: column; gap: 0; padding: 0; border: 1px solid var(--dsw-alias-border-l1); border-radius: 12px; background: var(--dsw-alias-bg-layer-1); overflow: hidden; box-shadow: 0 1px 3px rgba(16, 24, 40, 0.05); transition: border-color 140ms ease, box-shadow 140ms ease; }',
        '.dsh-mr-card:hover { border-color: color-mix(in srgb, var(--dsw-alias-border-l1) 55%, var(--dsw-alias-label-tertiary)); box-shadow: 0 2px 8px rgba(16, 24, 40, 0.08); }',
        '.dsh-mr-card-body { display: flex; flex-direction: column; gap: 10px; padding: 6px 14px 14px; animation: dsh-mr-body-in 160ms ease-out; }',
        '@keyframes dsh-mr-body-in { from { opacity: 0; transform: translateY(-4px); } to { opacity: 1; transform: none; } }',
        '@media (prefers-reduced-motion: reduce) { .dsh-mr-card-body { animation: none; } }',
        ...
        '.dsh-mr-card.is-open > .dsh-mr-card-head-btn { border-bottom: 1px solid var(--dsw-alias-border-l1); border-radius: 12px 12px 0 0; }',
        '.dsh-mr-card.is-open > .dsh-mr-card-head-btn:hover { background: none; }',
        '.dsh-mr-card-head-static { padding: 14px; }',
        '.dsh-mr-provtoolbar { display: flex; align-items: center; justify-content: space-between; gap: 8px; }',
        // 新增供应商表单
        ...
        // 状态条：常驻仪表盘行
        '.dsh-mr-strip { display: flex; align-items: center; flex-wrap: wrap; gap: 8px; padding: 10px 14px; border-radius: 10px; background: var(--dsw-alias-bg-layer-1); border: 1px solid var(--dsw-alias-border-l1); font-size: 12px; color: var(--dsw-alias-label-secondary); }',
        '.dsh-mr-strip-state { font-weight: 600; color: var(--dsw-alias-label-primary); }',
        '.dsh-mr-strip-state.is-on { color: #2AB67B; }',
        '.dsh-mr-strip.is-off { opacity: .75; }',
        '.dsh-mr-strip-sep { color: var(--dsw-alias-label-caption); }',
        '.dsh-mr-strip-stat { font-variant-numeric: tabular-nums; }',
        '.dsh-mr-strip-stat-warn { color: #A05A00; font-weight: 600; }',
        ...
        '.dsh-mr-caps-pickcaret { font-size: 10px; color: var(--dsw-alias-label-tertiary); }',
        ...
        '.dsh-mr-caps-badge { flex: none; font-size: 11px; line-height: 16px; padding: 0 6px; border-radius: 999px; color: var(--dsw-alias-label-tertiary); border: 1px solid var(--dsw-alias-border-l1); }',
        ...
        '.dsh-mr-chain { display: flex; flex-direction: column; gap: 6px; padding: 8px 10px 10px; border: 1px solid var(--dsw-alias-border-l1); border-left: 3px solid var(--dsw-alias-border-l2); border-radius: 10px; background: var(--dsw-alias-bg-layer-1); }',
        // tier 色轨：档位身份用颜色编码（PRO 紫 / NORMAL 蓝 / LITE 绿，与胶囊同源）
        '.dsh-mr-chain.is-tier3 { border-left-color: #9457E0; }',
        '.dsh-mr-chain.is-tier2 { border-left-color: #1890FF; }',
        '.dsh-mr-chain.is-tier1 { border-left-color: #2AB67B; }',
        '.dsh-mr-cand { transition: background 120ms ease; border-radius: 8px; }',
        '.dsh-mr-cand:hover { background: var(--dsw-alias-interactive-bg-hover); }',
        ...
        '.dsh-mr-ord { font-size: 11px; font-weight: 600; color: var(--dsw-alias-label-secondary); width: 18px; height: 18px; display: inline-flex; align-items: center; justify-content: center; border-radius: 50%; background: var(--dsw-alias-bg-layer-2); border: 1px solid var(--dsw-alias-border-l1); flex: none; }',
        ...
        '.dsh-mr-mini { padding: 3px 7px; border-radius: 6px; border: 1px solid var(--dsw-alias-border-l1); background: var(--dsw-alias-bg-layer-1); color: var(--dsw-alias-label-primary); font-size: 11px; cursor: pointer; line-height: 1.2; }',
        '.dsh-mr-mini:hover:not(:disabled) { border-color: var(--dsw-alias-brand-primary); color: var(--dsw-alias-brand-primary); }',
        '.dsh-mr-mini:active:not(:disabled), .dsh-mr-add:active:not(:disabled), .dsh-mr-chip-toggle:active:not(:disabled) { transform: translateY(1px); }',
        ...
        // 空档位单行：提示 + 添加按钮同行
        '.dsh-mr-chain-empty { display: flex; align-items: center; justify-content: space-between; gap: 8px; padding: 6px 10px; border: 1px dashed var(--dsw-alias-border-l1); border-radius: 8px; }',
