  ...
  factory: (require) => {
                      ...
                      React.createElement('div', { className: 'dsh-mr-caps-chips' },
                        THINKING_LEVELS.map(function (lv) {
                          ...
                          return React.createElement('div', {
                            className: 'dsh-mr-chip' + (enabled ? ' is-on' : ''),
                            key: lv,
                            role: 'button', tabIndex: 0,
                            'aria-pressed': enabled,
                            'aria-label': pid + '/' + m.id + ' 思考级别 ' + lv + (enabled ? '（已启用，点击取消）' : '（点击启用）'),
                            onClick: function (e) {
                              // 点在内部 wire 输入框上时不触发启用/取消
                              ...
                              setCapsDraft(function (dd) {
                                ...
                                if (enabled) {
                                  ...
                                  re[lv] = lv === 'off' ? null : (typeof re[lv] === 'string' ? re[lv] : lv)
                                }
    ...
    function apply(ctx) {
      ...
      styleEl.textContent = [
        ...
        '.dsh-mr-caps-chips { display: flex; flex-wrap: wrap; gap: 6px; }',
        '.dsh-mr-chip { display: inline-flex; align-items: center; gap: 5px; padding: 3px 9px; border-radius: 999px; border: 1px solid var(--dsw-alias-border-l1); background: transparent; color: var(--dsw-alias-label-secondary); font-size: 12px; cursor: pointer; user-select: none; transition: background .12s ease, border-color .12s ease, color .12s ease; }',
        '.dsh-mr-chip:hover { border-color: var(--dsw-alias-label-secondary); color: var(--dsw-alias-label-primary); }',
        '.dsh-mr-chip:focus-visible { outline: 2px solid var(--dsw-alias-brand-primary); outline-offset: 1px; }',
        '.dsh-mr-chip.is-on { background: var(--dsw-alias-interactive-bg-hover); border-color: var(--dsw-alias-brand-primary); color: var(--dsw-alias-label-primary); }',
        '.dsh-mr-chip-name { font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: 11px; font-weight: 600; line-height: 1.4; }',
        '.dsh-mr-chip-wire { width: 56px; padding: 1px 5px; border-radius: 6px; border: 1px solid transparent; background: transparent; color: inherit; font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: 11px; outline: none; }',
        '.dsh-mr-chip-wire:hover, .dsh-mr-chip-wire:focus { border-color: var(--dsw-alias-brand-primary); background: var(--dsw-alias-bg-layer-2); }',
