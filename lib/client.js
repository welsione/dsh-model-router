  ...
  factory: (require) => {
    ...
    function Panel() {
            ...
            return React.createElement('div', { className: 'dsh-mr-caps-prov', key: pid },
              ...
              list.map(function (m) {
                ...
                return React.createElement('div', { className: 'dsh-mr-caps-row', key: key },
                  ...
                  React.createElement('div', { className: 'dsh-mr-caps-fields' },
                    ...
                    React.createElement('div', { className: 'dsh-mr-caps-re' },
                      React.createElement('span', { className: 'dsh-mr-hint' }, '思考级别 reasoningEfforts'),
                      React.createElement('div', { className: 'dsh-mr-caps-regrid' },
                        THINKING_LEVELS.map(function (lv) {
                          var enabled = Object.prototype.hasOwnProperty.call(d.reasoningEfforts, lv) && d.reasoningEfforts[lv] !== null
                          var wireVal = Object.prototype.hasOwnProperty.call(d.reasoningEfforts, lv) ? d.reasoningEfforts[lv] : ''
                          return React.createElement('div', { className: 'dsh-mr-caps-lv', key: lv },
                            React.createElement('label', { className: 'dsh-mr-caps-lvchk' },
                              React.createElement('input', { type: 'checkbox', checked: enabled,
                                'aria-label': pid + '/' + m.id + ' 思考级别 ' + lv,
                                onChange: function (e) {
                                  var on = e.target.checked
                                  setCapsDraft(function (dd) {
                                    var base = dd[key] || capsInitDraft(pid, m.id)
                                    var re = Object.assign({}, base.reasoningEfforts || {})
                                    if (on) re[lv] = lv === 'off' ? null : (lv === wireVal ? lv : (typeof re[lv] === 'string' ? re[lv] : lv))
                                    else delete re[lv]
                            ...
                            enabled && lv !== 'off'
                              ? React.createElement('input', { type: 'text', className: 'dsh-mr-wire', value: wireVal || '',
                                  placeholder: 'wire…', name: 'wire', autocomplete: 'off', spellCheck: false,
                                  translate: 'no', 'aria-label': pid + '/' + m.id + ' 思考级别 ' + lv + ' wire 值',
                                  onChange: function (e) {
                                    var v = e.target.value
                                    setCapsDraft(function (dd) {
                                      var base = dd[key] || capsInitDraft(pid, m.id)
      ...
      var chainEditor = function (id, slot, label, hint) {
        ...
        var effortOptions = function (c) {
          var list = (efforts[c.provider + '/' + c.model] || []).slice()
          // 已保存的思考级别不在当前可选列表（含兜底）时，补一个临时项避免下拉显示空白
          if (c.reasoningEffort && !list.some(function (e) { return e.id === c.reasoningEffort })) {
            list.unshift({ id: c.reasoningEffort, name: c.reasoningEffort + '（已存）', verified: false })
          }
          return list
        }
            ...
            : chain.map(function (c, i) {
                ...
                return React.createElement('div', { className: 'dsh-mr-cand', key: i },
                  ...
                  React.createElement('select', {
                    ...
                    title: opts.length === 0 ? '不支持思考级别' : (opts.some(function (e) { return !e.verified }) ? '思考级别（该模型目录未标注，兜底档位未经验证）' : '思考级别'),
    ...
    function apply(ctx) {
      ...
      styleEl.textContent = [
        ...
        '.dsh-mr-caps-re { display: flex; flex-direction: column; gap: 4px; }',
        '.dsh-mr-caps-regrid { display: grid; grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); gap: 4px; }',
        '.dsh-mr-caps-lv { display: flex; align-items: center; gap: 5px; font-size: 12px; }',
        '.dsh-mr-caps-lvchk { display: inline-flex; align-items: center; gap: 4px; cursor: pointer; font-size: 12px; }',
        '.dsh-mr-caps-lvchk input { accent-color: var(--dsw-alias-brand-primary); }',
        '.dsh-mr-wire { width: 56px; padding: 2px 6px; border-radius: 5px; border: 1px solid var(--dsw-alias-border-l1); background: var(--dsw-alias-bg-layer-2); color: inherit; font-size: 11px; outline: none; }',
        '.dsh-mr-wire:focus-visible { outline: 2px solid var(--dsw-alias-brand-primary); outline-offset: 1px; }',
