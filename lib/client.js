...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Panel() {
      ...
      // 档位徽章：配色固定（pro/normal/lite 色系），显示名可自定义（cfg.tierNames）。
      // 用函数取显示名：自定义名优先，缺省用默认。
      var tierBadge = function (slot) {
        ...
        var custom = cfg && cfg.tierNames && cfg.tierNames[slot]
      ...
      return React.createElement('div', { className: 'dsh-mr' },
        ...
        React.createElement('div', { className: 'dsh-mr-card' },
          ...
          React.createElement('label', { className: 'dsh-mr-check', title: '稳定成功的候选优先，频繁失败的候选后移' },
            ...
            ' 健康度择优'),
          // 档位名称（自定义显示名，tier1/tier2/tier3）
          React.createElement('div', { className: 'dsh-mr-grid' },
            TIER_ORDER2.map(function (slot) {
              var dflt = { tier3: 'pro', tier2: 'normal', tier1: 'lite' }[slot]
              return React.createElement('label', { key: slot, title: '档位 ' + slot + ' 的显示名（默认 ' + dflt + '）' },
                slot + '（' + dflt + '）',
                React.createElement('input', { className: 'dsh-mr-text', type: 'text',
                  name: 'tierName-' + slot, autocomplete: 'off', spellCheck: false,
                  placeholder: dflt,
                  value: (cfg && cfg.tierNames && cfg.tierNames[slot]) || '',
                  onChange: function (e) {
                    var v = e.target.value.trim()
                    setCfg(function (p) {
                      var tn = Object.assign({}, (p.tierNames || {}))
                      if (v === '') delete tn[slot]
                      else tn[slot] = v
                      return Object.assign({}, p, { tierNames: tn })
                    })
                  } }))
            })),
    ...
    function PackageSelect({ available, directory, load, select, sessionId }) {
      ...
      var s10 = React.useState({});  var tierNames = s10[0]; var setTierNames = s10[1]
      ...
      // 档位显示名：自定义名优先，缺省用默认 lite/normal/pro
      var tierName = function (slot) {
        return (tierNames && tierNames[slot]) || TIER_META[slot] || slot
      }

      var refreshPkgs = React.useCallback(function () {
        if (!available) return
        api('/api/model-router/state').then(function (res) {
          setTierNames((res.config && res.config.tierNames) || {})
